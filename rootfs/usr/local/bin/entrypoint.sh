...
# @@File             :  entrypoint.sh
# @@Description      :  Entrypoint file for vim
...
#SERVICE_USER="${SERVICE_USER:-vim}"   # execute command as another user
#SERVICE_GROUP="${SERVICE_GROUP:-vim}" # Set the service group
# - - - - - - - - - - - - - - - - - - - - - - - - -
# Secondary ports
...
if [ "$ENTRYPOINT_FIRST_RUN" != "no" ]; then
  ...
  if [ "$CONFIG_DIR_INITIALIZED" = "no" ] || [ "$DATA_DIR_INITIALIZED" = "no" ]; then
    [ "$ENTRYPOINT_MESSAGE" = "yes" ] && echo "Executing entrypoint script for vim"
  fi
  # - - - - - - - - - - - - - - - - - - - - - - - - -
